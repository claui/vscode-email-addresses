# vscode-email-addresses

This is the source code repository for the `email-addresses`
VS Code extension.
...
4. Run the `yarn […] ovsx publish` command with the correct
   ...
   ```bash
   yarn workspace extension ovsx publish "dist/email-addresses-$(jq -r .version extension/share/dist/package.json).vsix"
   ```
...
```shell
yarn patch @vscode/vsce@npm:2.20.1
```
...
```shell
patch < path/to/this/project/.yarn/patches/@vscode-vsce-npm-2.20.1-c171711221.patch
```
...
- It may also use an incorrect key in the resolution entry it writes to `package.json`.  
  ...
  Example for a correct key: `"@vscode/vsce@^2.20.1"`
...
vscode-email-addresses depends.
...
vscode-email-addresses depends only indirectly, e.g. through a
...
all the way down to vscode-email-addresses), the patch may be
